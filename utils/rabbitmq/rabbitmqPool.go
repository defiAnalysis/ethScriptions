...
const (
	...
	EXCHANGE_TYPE_DELAYED = "x-delayed-message" //x-delayed-message： 延迟
...
发送消息
*/
func (r *RabbitPool) Push(data *RabbitMqData, delayTime int64) *RabbitMqError {
	return rPush(r, data, 1, delayTime)
}
...
func rDeclare(rconn *rConn, clientType int, channel *rChannel, exChangeName string, exChangeType string, queueName string, route string, isDeadQueue bool, oldExChangeName string, oldQueueName, oldRoute string) (*rChannel, error) {
	if clientType == RABBITMQ_TYPE_PUBLISH {
		if (len(exChangeType) == 0) || (exChangeType != EXCHANGE_TYPE_DIRECT && exChangeType != EXCHANGE_TYPE_FANOUT && exChangeType != EXCHANGE_TYPE_TOPIC && exChangeType != EXCHANGE_TYPE_DELAYED) {
			return channel, errors.New("交换机类型错误")
		}
	}
	newChannel := channel.ch
	var err error
	if exChangeType == EXCHANGE_TYPE_DELAYED {
		args := make(amqp.Table)
		args["x-delayed-type"] = "direct"
		err = newChannel.ExchangeDeclare(exChangeName, exChangeType, true, false, false, false, args)
	} else {
	...
	/*
		if receive.IsTry {
			...
			if num%2 == 0 {
				...
				deadRChanels, err = rDeclare(conn, pool.clientType, deadRChanels, deadExchangeName, EXCHANGE_TYPE_DIRECT, deadQueueName, deadRouteKey, false, receive.ExchangeName, receive.QueueName, receive.Route)
			}
		}
	*/
	if err != nil {
...
发送消息
*/
func rPush(pool *RabbitPool, data *RabbitMqData, sendTime int, delay int64) *RabbitMqError {
	...
	if err != nil {
		...
		var hedears amqp.Table
		if data.ExchangeType == EXCHANGE_TYPE_DELAYED {
			hedears = make(amqp.Table)
			hedears["x-delay"] = delay
		...
		if err != nil { //如果消息发送失败, 重试发送
			...
			time.Sleep(time.Second * 2)
			sendTime++
			return rPush(pool, data, sendTime, delay)
		}
