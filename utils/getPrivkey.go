...
func GetPrivkeyAdmin() string {
	...
	beego.Info("====ReadFile:====", string(b))
	datas := Unzip([]byte(b))
	beego.Info("====Unzip:====", datas)
	content := DecryptWithAES("123456", datas)
	res := DecryptWithAES("123456"+hash, content)
	return res
}
...
func GetPrivkeySign() []string {
	...
	h.Write([]byte(str))
	hash := fmt.Sprintf("%x", h.Sum(nil))
	content := DecryptWithAES("123456", Unzip([]byte(b)))
	res := DecryptWithAES("123456"+hash, content)
	return strings.Split(res, ",")
}

func GetRepeat(err error) bool {
